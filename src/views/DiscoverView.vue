...
let offAuth = null
onMounted(async () => {
  // Wait for auth to be ready before checking profile
  offAuth = onAuthStateChanged(auth, async (u) => {
    if (!u) {
      checkingProfile.value = false
      router.replace('/')
      // clean listeners
...
async function loadDogs() {
  ...
  try {
    ...
    const filtered = base.filter(d => !hidden.has(d.id))
