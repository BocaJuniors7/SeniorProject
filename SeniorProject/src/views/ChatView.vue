...
async function selectConversation(conv) {
  try {
    ...
    console.log('Current dog ID:', currentDogId.value)
    ...
    // Verify we own the current dog
    const myDogDoc = await getDoc(doc(db, 'dogs', currentDogId.value))
    if (!myDogDoc.exists()) {
      console.error('Current dog does not exist:', currentDogId.value)
      alert('Selected dog does not exist')
      return
    }
    const myDogData = myDogDoc.data()
    console.log('Current dog owner:', myDogData.ownerId, 'Current user:', auth.currentUser?.uid)
    if (myDogData.ownerId !== auth.currentUser?.uid) {
      console.error('User does not own current dog')
      alert('You do not own the selected dog')
      return
    }
    ...
    if (conv.type === 'new') {
      ...
      try {
        ...
        console.log('Checking for existing thread with membersKey:', membersKey)
        const existing = await getDocs(
          query(collection(db, 'threads'), where('membersKey', '==', membersKey))
        )
        console.log('Query result:', existing.empty ? 'no existing thread' : 'found thread')
        ...
        if (!existing.empty) {
          ...
          // Create new thread
          console.log('Creating new thread for dogs:', currentDogId.value, otherDogId)
          const threadData = {
          ...
          // Only include these fields if they have values (avoid null/empty issues)
          console.log('Thread data to create:', threadData)
        ...
        console.error('Error querying/creating thread:', queryError)
        console.error('Error code:', queryError.code)
        console.error('Error message:', queryError.message)
        console.error('Full error:', queryError)
        throw new Error(`Failed to access threads: ${queryError.message} (Code: ${queryError.code})`)
      }
    } else {
      finalThreadId = conv.threadId
      console.log('Using existing threadId from conversation:', finalThreadId)
    }
