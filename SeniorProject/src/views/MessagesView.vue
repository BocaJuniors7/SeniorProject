    ...
    <div class="messages-layout">
      ...
      <aside class="convo-list">
        ...
        <button
          ...
          @click="handleConversationClick(c.key, $event)"
          type="button"
        >
          <img :src="c.dog.image || placeholder" alt="" class="convo-avatar" />
          <div class="convo-meta">
            <div class="convo-top">
              <span class="convo-name">{{ c.dog.name || c.dog.ownerName || 'Unknown' }}</span>
...
const selectedConversation = computed(() => {
  const found = conversations.value.find(c => c.key === selectedConvKey.value)
  console.log('Selected conversation computed:', {
    selectedConvKey: selectedConvKey.value,
    found,
    allConversations: conversations.value.map(c => ({ key: c.key, type: c.type, dogName: c.dog?.name }))
  })
  return found
})
const selectedDog = computed(() => {
  const dog = selectedConversation.value?.dog || null
  console.log('Selected dog computed:', dog)
  return dog
})
...
// Simple click handler to ensure it fires
function handleConversationClick(key, event) {
...
async function openConversation(key) {
  try {
    console.log('Opening conversation:', key)
    const item = conversations.value.find(c => c.key === key)
    if (!item) {
      console.error('Conversation item not found for key:', key, 'Available conversations:', conversations.value)
      return
    }

    console.log('Found conversation item:', item)

    // Resolve threadId (create if "new")
    let threadId = null
    if (item.type === 'new') {
      console.log('Creating new thread for dog:', item.dog.id)
      threadId = await ensureThread(item.dog.id)
      selectedConvKey.value = `t:${threadId}`
    ...
    console.log('Thread ID:', threadId, 'Current dog ID:', currentDogId.value)
    ...
    if (!currentDogId.value) {
      console.error('No current dog ID selected')
      return
    }

    // 🔒 Guard: if the match is gone, purge now and bail
    // But don't block if there's an error - let the user try to open it anyway
    try {
      const myId = currentDogId.value
      const otherId = item.dog.id
      if (myId && otherId && threadId) {
        const deleted = await purgeIfUnmatched(threadId, myId, otherId)
        if (deleted) {
          console.log('Thread was purged (unmatched)')
          // Clear selection if thread was deleted
      ...
      // Continue anyway - don't block the user from opening the conversation
    }
    ...
    await nextTick()
    console.log('Conversation opened successfully. Selected dog:', selectedDog.value)
    
    // Double-check that selectedDog is set
    if (!selectedDog.value) {
      console.warn('Selected dog is null after opening conversation. Forcing update...')
      // Force a re-computation by updating the key
      const currentKey = selectedConvKey.value
      selectedConvKey.value = null
      await nextTick()
      selectedConvKey.value = currentKey
    }
  } catch (error) {
...
async function purgeIfUnmatched(threadId, myDogId, otherDogId) {
  try {
    ...
    // Only purge if we're certain the match is broken
    // Don't purge if there's any error checking
    ...
    console.error('Error in purgeIfUnmatched:', error)
    // Don't purge if there's an error - might be a temporary issue
    return false
  }
}
...
watch(currentDogId, async (n) => {
  ...
  // Handle route query params after dog is set
  await handleRouteQuery()
})

// Handle route query params (e.g., ?with=dogId&fromDog=myDogId)
async function handleRouteQuery() {
  ...
  if (withDogId && fromDogId && currentDogId.value === fromDogId) {
    // Find or create conversation with this dog
    const match = fetchedMatches.value.find(m => m.id === withDogId)
    if (match) {
      const tid = await ensureThread(withDogId)
      selectedConvKey.value = `t:${tid}`
      selectedThreadId.value = tid
      subscribeMessages(tid)
...
onMounted(async () => { 
  ensureSignedIn()
  // Wait a bit for auth and dogs to load, then handle route
  await nextTick()
  setTimeout(() => handleRouteQuery(), 500)
})
onUnmounted(() => {
  if (unsubscribeAuth) { unsubscribeAuth(); unsubscribeAuth = null }
  if (unsubscribeThreads) { unsubscribeThreads(); unsubscribeThreads = null }
