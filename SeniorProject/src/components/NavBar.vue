      ...
      <div class="nav-links">
        <template v-if="user">
          ...
          <div class="notification-wrapper" @click="goToNotifications">
            ...
            <span v-if="totalNotifications > 0" class="notification-badge">{{ totalNotifications > 99 ? '99+' : totalNotifications }}</span>
          </div>
        </template>
        <template v-else>
...
import { totalNotifications } from '../services/notifications'
...
const goToNotifications = () => {
  // Navigate to messages page (where notifications are most relevant)
